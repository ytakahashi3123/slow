...
- `config.yml` の `lusgs_beta` と `kind_lusgs_dissipation` に注意書きを追加した。
  `matrix` を**定常計算**で使うとき、格子と `courant_number` の組み合わせによっては
  残差が下がったあと再上昇する。原因は対角重みの不足なので、**`courant_number` を
  下げる**か **`lusgs_beta` を上げる**かのどちらでも解消する
  - 実測（`beta: 1.1`）: chimera 格子は CFL 5.0 で反復 78 の 6.2e5 から 2e9 へ再上昇するが、
    **CFL 2.5 なら 168 反復・残差 5.1e1 で収束する**（CFL 5.0 のまま `beta: 2.0` にした
    場合の 230 反復・5.9e1 より良い）。CFL 10/20 は破綻、CFL 1.0 は単調だが遅い
  - 実測: ノズル格子は CFL 2.5 で 82 反復で負の密度・温度・圧力になって停止するが、
    **CFL 1.0 なら 300 反復を単調に完走する**（2.90e6。`scalar` の 3.19e6 より良い）。
    ただしこの格子は `scalar` でも定常では 3e6 程度で停滞するので定常向きではない
  - 一方 **wedge 格子では CFL 2.5・既定値でも単調で `scalar` より速い**
  - `scalar` は同じ chimera 格子で CFL 20 まで再上昇しない。両者の差は
    「許容できる CFL の幅」として現れる
    （300 反復で 1.5e12 対 2.9e12）。なおノズルは `scalar` でも定常では反復 240 で停滞して
    1.7 倍上昇するので、そもそも定常向きの格子ではない。tutorial のうち定常なのは
    `work_wedge` だけなので、出荷している設定でこの組み合わせになるものは無い
