...
**`matrix` を定常計算で使うときは残差の履歴を確認すること。** 格子・条件によっては
既定の `lusgs_beta` (1.01--1.1) で残差が下がったあと再上昇する
（chimera 格子は反復 78 から、ノズル格子は反復 73 から。一方 wedge 格子では
既定値でも単調で `scalar` より速い）。再上昇していたら `lusgs_beta` を 2.0 程度まで
上げる（chimera で 230 反復・残差 5.9e1 まで収束し、`scalar` の 5.4e1 と同水準になる）。
